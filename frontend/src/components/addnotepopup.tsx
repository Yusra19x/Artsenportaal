...
const AddNotePopup: React.FC<AddNotePopupProps> = ({
    ...
    // ?? Vul velden automatisch in bij bewerken
    useEffect(() => {
        if (editNote) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Voeg nieuwe notitie toe of bewerk bestaande
            if (editNote && onNoteEdited) {
                onNoteEdited(editNote.id, title.trim(), content.trim());
            } else if (onNoteAdded) {
                notesService.createNote(patientId, patientName, title.trim(), content.trim());
                onNoteAdded(title.trim(), content.trim());
            }
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
                <div className="p-6">
                    {/* Titel: Nieuw of Bewerken */}
                    <div className="flex justify-between items-center mb-4">
                        <h2 className="text-xl font-bold text-gray-900">
                            {editNote ? 'Notitie bewerken' : 'Nieuwe notitie'}
                        </h2>
                        <button
                            onClick={handleClose}
                    ...
                    <div className="mb-4">
                        <p className="text-sm text-gray-600">
                            Notitie voor:{' '}
                            <span className="font-medium text-blue-600">{patientName}</span>
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="flex justify-end space-x-3 pt-4">
                            ...
                            <button
                                ...
                                disabled={saving}
                            >
                                {saving ? 'Opslaan...' : editNote ? 'Wijzigingen opslaan' : 'Notitie opslaan'}
                            </button>
                        </div>
                    </form>
                </div>
